...
class master_interviewer{
	public:
		...
		void feedback(assistant_interviewer* assistant[]){
		...
		void do_interview(assistant_interviewer* assistant[]){
			...
			set_topic();
			for(int i = 0; i <= topic_num; i++){
				make_topic_question(i);
				cout << endl << endl << question[question.size() - 1] << std::endl;
				string a;
				getline(cin, a);
				ans.push_back(a);
				make_comment();
				...
				for(int j = 0; j < tail_num; j++){
					make_tail_question();
					cout << endl << endl;
					cout << question[question.size() - 1] << endl;
					string a;
					getline(cin, a);
					ans.push_back(a);
					make_comment();
			...
			feedback(assistant);
			std::cout << "\n\nNow, the mock interview is over. I hope this mock interview helped you prepare for your interview. Goodbye~";
		}
	...
	private:
		std::vector<string> question, ans, comment;
		std::vector<string> topic_list;
		std::vector<bool> topic_done;
		int topic_num;
};
